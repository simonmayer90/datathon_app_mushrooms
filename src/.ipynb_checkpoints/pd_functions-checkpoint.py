...
def get_ready_test(RESULTS_PATH: str, uploaded_file):
    ...
    if(test.shape != (1625, 2) ):
        ...
        return 0
    if((test.poisonous.unique().tolist() != [0,1] ) & (test.poisonous.unique().tolist() != [1,0] ) & (test.poisonous.unique().tolist() != [1] ) & (test.poisonous.unique().tolist() != [0] ) ):
...
def get_metrics(RESULTS_PATH: str, test: pd.DataFrame):
    ...
    results
        ...
        .assign(
            tp = lambda df_: np.where((df_['real']==1) & (df_['preds']==1), True, False),
            correct = lambda df_: df_['real'] == df_['preds'], 
            fn = lambda df_: np.where((df_['real']==1) & (df_['preds']==0), True, False),
    ...
    pd.DataFrame([[
        ...
        row_evaoluation['tp']/(row_evaoluation['tp'] + row_evaoluation['fn']),  
